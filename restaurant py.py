...
lblInfo=Label(Tops,font=('helvetica',50,'bold'),text="Suraj's Restaurant",fg="Black",bd=10,anchor='w')
lblInfo.grid(row=0,column=0)

lblInfo=Label(Tops,font=('arial',20,'bold'),text=dt_string,fg="red",bd=10,anchor='w')
...
def Ref():
    ...
    if (Noodles.get()==""):
        ...
        Countof_Noodles=float(Noodles.get())
    ...
    if (Burger.get()==""):
       ...
       Countof_Burger=float(Burger.get())

        
    if (Sandwich.get()==""):
       Countof_Sandwich=0
    else:
       Countof_Sandwich=float(Sandwich.get())

     
    if (Drinks.get()==""):
       Countof_Drinks=0
    else:
       Countof_Drinks=float(Drinks.get())

                   
    CostofFries =Countof_Fries * 70
    CostofDrinks=Countof_Drinks * 30
    CostofNoodles = Countof_Noodles* 50
    CostofSoup = Countof_Soup * 30
    CostBurger = Countof_Burger* 40
    CostSandwich=Countof_Sandwich * 40

    CostofMeal= "Rs",str('%.2f' % (CostofFries+CostofDrinks+CostofNoodles+CostofSoup+CostBurger+CostSandwich))

    PayTax=((CostofFries+CostofDrinks+CostofNoodles+CostofSoup+CostBurger+CostSandwich) * 0.10)
